...
export async function GET() {
  ...
  const [{ data: alerts }, { data: transactions }, { data: holdings }] = await Promise.all([
    supabase.from("alerts").select("id").eq("user_id", user.id).eq("status", "active"),
    supabase.from("transactions").select("coin_id, symbol, type, amount, price, total_value, date").eq("user_id", user.id).eq("status", "completed"),
  ...
  for (const tx of txs) {
    if (!coinMeta[tx.coin_id]) coinMeta[tx.coin_id] = { symbol: tx.symbol, spent: 0 };
    if (tx.type === "buy" || tx.type === "deposit") coinMeta[tx.coin_id].spent += Number(tx.total_value);
    if (tx.type === "sell" || tx.type  === "withdrawal") coinMeta[tx.coin_id].spent -= Number(tx.total_value);
  }
  const totalSpent = Object.values(coinMeta).reduce((sum, c) => sum + c.spent, 0);

  const netPnL = totalValue - totalSpent;
  const pnlPercent = totalSpent > 0 ? ((netPnL / totalSpent) * 100).toFixed(2) : 0;
  ...
  for (const [coinId, amount] of Object.entries(holdingsMap)) {
    ...
    const percent = avgBuy > 0 ? (((livePrice - avgBuy) / avgBuy) * 100).toFixed(2) : 0;
  ...
  for (const tx of txs) {
    ...
    const delta = (tx.type === "buy"||  tx.type === "deposit") ? Number(tx.total_value) : (tx.type === "sell" || tx.type === "withdrawal") ? -Number(tx.total_value) : 0;
