...
interface AlertListProps {
  ...
  onSelect: (alert: EnrichedAlert) => void;
  ...
  return (
    <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col">
      ...
      <div className="flex flex-col gap-2">
        {alerts.map((alert) => {
          const color = coinColors[alert.coin_symbol] ?? defaultColor;
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  <button
                    ...
                    className={`w-10 h-6 rounded-full transition-colors relative shrink-0 cursor-pointer ${isActive ? "bg-primary" : "bg-white/10"}`}
                  >
                    <span
                      className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${isActive ? "left-5" : "left-1"}`}
                    />
                  </button>
                  <button
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/5">
                  ...
                  {alert.last_price && (
                    <span className="text-[11px] text-white/30">
                      Last price: ${alert.last_price?.toLocaleString()}
                    </span>
                  )}
                </div>
              )}
