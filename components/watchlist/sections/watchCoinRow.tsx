...
function Pct({ value }: { value: number }) {
  const pos = value >= 0
  return (
    <span className={`flex items-center gap-0.5 text-sm font-medium ${pos ? 'text-green-400' : 'text-red-400'}`}>
...
export function CoinRow({ coin, onToggleStar, onToggleAlert }: CoinRowProps) {
  const color = coinColors[coin.symbol] ?? defaultColor

  return (
    <tr className="border-t border-white/5 hover:bg-white/5 transition-colors group">
      ...
      <td className="py-4 pr-4 hidden sm:table-cell">
        <Sparkline data={coin.sparkline} positive={coin.price_change_percentage_7d >= 0} />
      </td>
      ...
      <td className="py-4 pr-2">
        <div className="flex items-center gap-2">
          <button
            onClick={() => onToggleStar(coin.id)}
            className="p-1.5 rounded-lg hover:bg-white/10 transition-colors"
          >
            <Star className={`w-4 h-4 transition-colors ${coin.isWatchlisted ? 'text-yellow-400 fill-yellow-400' : 'text-white/20 hover:text-yellow-400'}`} />
          </button>
          <button
            onClick={() => onToggleAlert(coin.id)}
            className="p-1.5 rounded-lg hover:bg-white/10 transition-colors"
          >
            <Bell className={`w-4 h-4 transition-colors ${coin.hasAlert ? 'text-primary fill-primary/20' : 'text-white/20 hover:text-primary'}`} />
          </button>
        </div>
      </td>
    </tr>
